...
class plants_list_model extends CI_Model
{
	...
	* 更新資料
	**/
	public function updateList($inId, $inData, $inImageData)
	{	
		...
		foreach ($inImageData as $key => $value)
		{
			if(is_int($key))
			{
				if(isset($value['file_name']))array_push($imageData, $value['file_name']);
			}else{
				if(isset($value['file_name']))$data[$key] = $value['file_name'];
			}
		}
		$data['pl_image'] = json_encode($imageData);
		foreach($inData as $key => $value)
		{
			if($key == 'type' || $key == 'id')continue;
			$data[$key] = $value;
		}
		if($data['pl_image'][0] == '')$data['pl_image'] = '';
		if($data['pl_name1_img'][0] == '')$data['pl_name1_img'] = '';
		if($data['pl_name2_img'][0] == '')$data['pl_name2_img'] = '';
		
		$this->db->update('plants_list', $data, "pl_id = ". $inId);
	...
	**/
	public function addList($inData, $inImageData)
	{
		...
		if($data['pl_name1_img'][0] == '')$data['pl_name1_img'] = '';
		if($data['pl_name2_img'][0] == '')$data['pl_name2_img'] = '';
		$this->db->insert('plants_list', $data);
		if ($this->db->affected_rows() > 0) {
			return "SUCCESS";
		} else {
			return "FAIL";
	...
	**/
	...
	{
		$this->db->where('pl_id', $inId);
		$this->db->delete('plants_list');
