						...
						foreach ($result as $key => $value) {
							...
							if($imageArray != NULL)
							{
								foreach ($imageArray as $key2 => $value2)
								{
									$isHidden = ($i > 0)?'hide':'';
									$imageDom .= '
										<a href="../uploads/'. $value2 .'" class="lightview '. $isHidden .'  imgBox_'. $value['pl_id'] .'" 
							...
							if($value['pl_name1_img'] == '')
							{
								...
								$name1 = '<a href="../uploads/'. $value['pl_name1_img'] .'" class="lightview imgBox1_'. $value['pl_id'] .'" 
							...
							if($value['pl_name2_img'] == '')
							{
								...
								$name2 = '<a href="../uploads/'. $value['pl_name2_img'] .'" class="lightview imgBox2_'. $value['pl_id'] .'" 
										data-lightview-group-options="skin: \'mac\'">
										'. $value['pl_name2'] .'</a>';
							}
							...
							echo '
								<tr>
									...
									<td>
										<input type="hidden" name="pl_id" value="'. $value['pl_id'] .'"/>
										<input type="hidden" name="pl_type" value="'. $value['pl_type'] .'"/>
										<input type="hidden" name="pl_name1" value="'. $value['pl_name1'] .'"/>
										<input type="hidden" name="pl_name2" value="'. $value['pl_name2'] .'"/>
										<input type="hidden" name="pl_name1_img" value="'. $value['pl_name1_img'] .'"/>
										<input type="hidden" name="pl_name2_img" value="'. $value['pl_name2_img'] .'"/>
										<input type="hidden" name="pl_size" value="'. $value['pl_size'] .'"/>
										<input type="hidden" name="pl_price" value="'. $value['pl_price'] .'"/>
										<input type="hidden" name="pl_image" value="'. $value['pl_image'] .'"/>
										<input type="hidden" name="pl_comment" value="'. $value['pl_comment'] .'"/>
	...
	<div class="panel panel-primary" id="plants_popup">
		<div class="panel-heading text-center">新增植株列表<span class="glyphicon glyphicon-remove pull-right cursor" onClick="javascript:$.unblockUI();"></span></div>
		<div class="panel-body">
			<form class="dom_form" name="image" method="POST" enctype="multipart/form-data">
				<!-- <input type="hidden" value="1" name="type"/> -->
				<input type="hidden" name="id" value=""/>
				<input type="hidden" name="pl_type" value="0"/>
				<div class="row">
				...
				<div class="row">
					...
					<div class="col-lg-4">
						<input type="text" class="form-control required " value="" name="pl_size"/>
					</div>
				</div>
				<br/>
				<div class="row">
