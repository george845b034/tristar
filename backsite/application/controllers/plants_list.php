...
class Plants_list extends CI_Controller
{
	private $message;
	...
	{	
		...
		switch ($type) {
			...
			//編輯圖片
			case 3:
				echo json_encode($this->editImage());
				exit();
			break;
			//分頁
			case 4:
	...
	private function getList()
	{
		...
		$result['result'] = $this->plants_list_model->getListContent(0, $start, $limit, $sortId, $sort);
	...
	**/
	private function getPage()
	{
		...
		{	
			...
			$count = $this->plants_list_model->getListBasic(0);
		}
		$tempArray = Array(
	...
	private function editImage()
	{
		...
		$result['status'] = $this->plants_list_model->updateList($this->input->post("id"), $this->input->post(NULL, TRUE), $data);
	...
	**/
	private function view_page($inData)
	{
		...
		$this->load->view('plants_list', $inData);
